...
%% Setting ILMI and IMAX
ILMI= leak(PIDY.I,RFND.Dist,RFND.TimeUS);% For Yaw
IMAX= max(abs(PIDY.I));% For Yaw

%% FeedForward Calculation
Max_FeedForward = Feedforward(RATE.Y,RATE.YOut,RATE.TimeUS,filename)

%% Comparing two signals
[C,RMSE,L] = correlation(des_sgnl,act_sgnl,RFND.Dist,ATT.TimeUS);

%% Calculating Time Constant
ATC_Input_TC_YAW = time(ATT.Yaw,ATT.DesYaw,filename);%from yaw
ATC_Input_TC_YAW = time(ATT.Roll,ATT.DesRoll,filename);%from roll
ATC_Input_TC_YAW = time(ATT.Pitch,ATT.DesPitch,filename);%from pitch
