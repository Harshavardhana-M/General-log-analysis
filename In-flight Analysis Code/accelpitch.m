Ydes_accel=diff(double(RATE.PDes))./diff((10^-6)*double(RATE.TimeUS));%Differentiation of desired RATE
YAct_accel=diff(double(RATE.P))./diff((10^-6)*double(RATE.TimeUS));%Differentiation of RATE
...
Max_AccelPDes_back=max(double(filt_YDes))
Max_AccelPAct_back=max(double(filt_YAct))

%Maximum Negative Accelerations 
Max_AccelPDes_forw=min(double(filt_YDes))
Max_AccelPAct_forw=min(double(filt_YAct))
