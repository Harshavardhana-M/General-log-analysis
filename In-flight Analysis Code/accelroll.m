Ydes_accel=diff(double(RATE.RDes))./diff((10^-6)*double(RATE.TimeUS));%Differentiation of desired RATE
YAct_accel=diff(double(RATE.R))./diff((10^-6)*double(RATE.TimeUS));%Differentiation of RATE
...
Max_AccelRDes_Right=max(double(filt_YDes))
Max_AccelRAct_Right=max(double(filt_YAct))

%Maximum Negative Accelerations 
Max_AccelRDes_Left=min(double(filt_YDes))
Max_AccelRAct_Left=min(double(filt_YAct))
